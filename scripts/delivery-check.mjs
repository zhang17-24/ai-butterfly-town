...
const mustHave = ["Dockerfile", "docker-compose.yml", "render.yaml", ".env.example", "apps/server/.env.example", "docs/delivery.md", "docs/agents/README.md"];
const missing = mustHave.filter((file) => !existsSync(path.join(root, file)));
if (missing.length > 0) {
  console.error(`[FAIL] 部署/文档文件缺失:${missing.join(", ")}`);
  process.exit(1);
}
console.log(`部署/文档文件齐备(${mustHave.length} 项)。`);
...
const checklist = [
  ...
  "Docker 单服务可启动:cp .env.example .env && docker compose up --build 后访问 :3100(共享库 dist 导出已修复)",
  "云部署(至少一种):Render Blueprint(render.yaml 已备)或任意主机 docker compose",
