...
| D125–D132 | 三天开发路线、参考边界与当前实装顺序 | 全里程碑状态管理 |
...
**M4-1 完成证据**：玩家实体及位置持久化、Blueprint 室外碰撞网格、服务端 A*、版本化幂等移动命令、客户端逐格插值、跨桥、非法目标提示和刷新恢复已落地；自动化与 Ego Lite 长路径/Tick 竞态回归通过。

### 任务顺序

1. [x] 在 World State 增加玩家实体、默认角色与当前位置持久化；
2. [x] Blueprint 落地首版室外网格、建筑/河流障碍和桥梁通路；
3. [x] 实现服务端权威 A*：目标合法性、路径计算与版本化原子移动命令；
4. [x] Ego Lite 验证地图点击、客户端路径插值、跨桥、非法目标提示和刷新恢复；
5. [ ] 远程查看不移动，现场动作自动接近；
6. [ ] 实现对话会话、参与者锁和紧急中断；
7. [ ] 接入 SIMULATION 角色的对话 Schema；
8. [ ] 实现 Mock 对话的意图/提及/人格/心情/关系模板；
9. [ ] 每次对话写事件，提取近期记忆并小幅更新有向关系；
10. [ ] 对话抽屉提供自由输入与至少 3 个上下文动作。
