...
- Test: `apps/web/src/game3d/voxel/actorSpecs.test.ts`

**Interfaces:**
- Consumes: Task 4 的 `VoxelActorSpec` / `buildVoxelActor`
- Produces: `ACTOR_SPECS: Record<string, VoxelActorSpec>`，键为 `npc_lin_xia` / `npc_shen_zhiheng` / `npc_he_jianguo` / `npc_zhou_fang` / `npc_tang_yucheng` / `player`；以及 `fallbackSpec(id: string, color: string): VoxelActorSpec` / `actorSpecFor(id: string, color: string): VoxelActorSpec`

**配色来源**：`TownScene.ts:447-453` 的 per-NPC palette（2D 已验证过的配色，3D 原样沿用）；衣色用 `NpcProfile.color` 之外的固定值以便与 2D 一致。

> **关于调色板约束的适用范围**（重要，别搞反）：
> `qixiPixelStyle.palette` 那 8 色是**地图/建筑**的锁定调色板（水体、草地、路面、砖墙），里面没有肤色和发色。
> 角色配色来自 2D 的 `createPixelAvatar` 表，那是一套独立且更丰富的颜色（`#f2bd91` 肤色、`#3f2722` 发色等），
> 2D 已经逐帧验收过。所以 Task 5 的测试**不能**断言"角色颜色 ∈ qixiPixelStyle.palette" —— 那两个要求互相矛盾。
> 角色这一侧的约束是**与 2D 同一个人**：肤色/发色/强调色必须与 `TownScene.ts:447-453` 的表逐项相等。

- [ ] **Step 1: 写失败的测试**

创建 `apps/web/src/game3d/voxel/actorSpecs.test.ts`：
...
Run: `pnpm --filter @ai-town/web exec vitest run actorSpecs`
Expected: FAIL — `Cannot find module './actorSpecs'`
...
```bash
git add apps/web/src/game3d/voxel/actorSpecs.ts apps/web/src/game3d/voxel/actorSpecs.test.ts
